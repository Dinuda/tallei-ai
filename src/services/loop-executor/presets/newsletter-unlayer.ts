/**
 * newsletter-unlayer.ts — backend Unlayer design generation for newsletter runs.
 *
 * The dashboard can load the returned design in the Unlayer editor, while
 * delivery can use the rendered HTML immediately without waiting for UI export.
 */
...
function readSubjectLine(line: string): string | null {
  const match = line.trim().match(/^subject\s*:\s*(.+)$/i);
  return match?.[1]?.trim() ? match[1].trim() : null;
}

function readPreviewLine(line: string): string | null {
  const match = line.trim().match(/^preview\s*:\s*(.+)$/i);
  return match?.[1]?.trim() ? match[1].trim() : null;
}

function parseNewsletterMarkdown(markdown: string, fallbackSubject: string | null | undefined) {
  ...
  for (const line of lines) {
    const subjectLine = readSubjectLine(line);
    if (subjectLine) {
      subject ||= subjectLine;
      continue;
    }
    const previewLine = readPreviewLine(line);
    if (previewLine) {
      preview ||= previewLine;
      continue;
    }
  ...
  if (!subject) {
    const firstContent = body.split("\n").find((line) => line.trim())?.trim() ?? "Newsletter update";
...
function markdownBlocks(markdown: string): UnlayerRow[] {
  ...
  function flushText() {
    ...
    textBuffer = [];
    if (!text) return;
    rows.push(makeRow(`body-${rowIndex++}`, [
      textContent(`text-body-${rowIndex}`, `<p style="line-height:170%;">${inlineMarkdown(text)}</p>`),
    ]));
  }
  ...
  for (const rawLine of markdown.split("\n")) {
    ...
    if (heading) {
      ...
      rows.push(makeRow(`heading-${rowIndex++}`, [headingContent(`heading-${rowIndex}`, escapeHtml(heading[2]), level)]));
      continue;
    }
    if (/^[-*]\s+/.test(line)) {
      flushText();
      rows.push(makeRow(`bullet-${rowIndex++}`, [
        textContent(`text-bullet-${rowIndex}`, `<p style="line-height:170%;margin:0;">• ${inlineMarkdown(line.replace(/^[-*]\s+/, ""))}</p>`),
      ]));
      continue;
    }
...
export function buildUnlayerNewsletterEmail(input: {
  markdown: string;
  subject?: string | null;
}): { design: UnlayerDesign; html: string; text: string; subject: string; preview: string } {
  const parsed = parseNewsletterMarkdown(input.markdown, input.subject);
  const today = new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }).toUpperCase();
  const rows = [
    makeRow("preheader", [textContent("text-preheader", `<p style="line-height:140%;">${escapeHtml(parsed.preview)}</p>`, { fontSize: "13px", color: "#6b7280", align: "center" })], "8px 0px"),
    ...
    ...markdownBlocks(parsed.body),
    makeRow("footer-divider", [{ id: "divider-footer", type: "divider", values: { containerPadding: "22px 30px 8px", border: { borderTopWidth: "1px", borderTopStyle: "solid", borderTopColor: "#e5e7eb" } } }]),
    makeRow("footer", [textContent("text-footer", '<p style="line-height:150%;">You’re receiving this because you subscribed to updates from Tallei.<br /><a href="{{{RESEND_UNSUBSCRIBE_URL}}}" style="color:#6b7280;text-decoration:underline;">Unsubscribe</a></p>', { fontSize: "12px", color: "#9ca3af", align: "center" })]),
  ];
  const design: UnlayerDesign = {
    ...
    body: {
      id: "backend-unlayer-newsletter-body",
      rows,
      values: {
        backgroundColor: "#ffffff",
        contentWidth: "640px",
  ...
  const html = renderUnlayerNewsletterHtml(design, parsed.subject, parsed.preview);
  const text = [parsed.subject, "", parsed.body, "", "Unsubscribe: {{{RESEND_UNSUBSCRIBE_URL}}}"].join("\n");
  return { design, html, text, subject: parsed.subject, preview: parsed.preview };
}

function renderUnlayerNewsletterHtml(design: UnlayerDesign, subject: string, preview: string): string {
