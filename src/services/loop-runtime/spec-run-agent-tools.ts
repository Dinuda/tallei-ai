...
export function buildAgentTools(input: BuildAgentToolsInput): Record<string, Tool> {
  ...
  tools.requestReview = tool({
    description: "Create a first-class operator artifact review surface. Use for draft, email, preview, sources, and memories review. Pass artifactData as a plain JSON object (NOT a stringified JSON string).",
    inputSchema: z.object({
      surface: inputSurfaceSchema,
  ...
  tools.requestApproval = tool({
    ...
    execute: async ({ actionRef, payload, rationale, artifactKey }) => {
      ...
      const enriched = enrichDraftPayload(writeTool.actionSlug, payload, input.runContext);
        ...
        if (input.auth.workspaceId) {
          try {
            await persistLoopRunWorkspaceMemory(input.auth, {
              workflowId: input.workflowId,
              runId: input.runId,
              workflowTitle: input.spec.title,
              approvedMemories: [],
