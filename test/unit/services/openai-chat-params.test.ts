...
test("openAiModelSupportsCustomTemperature rejects nano and reasoning models", () => {
  ...
  assert.equal(openAiModelSupportsCustomTemperature("gpt-gpt-5-nano"), false);
...
test("openAiTemperatureParam omits temperature for fixed-temperature models", () => {
  ...
  assert.deepEqual(openAiTemperatureParam("gpt-gpt-5-nano", 0), {});
  assert.deepEqual(openAiTemperatureParam("gpt-gpt-5-nano"), {});
});
