...
1. **Prompt drift.** System prompts, temperature behaviour, JSON-mode support, and context window sizes differ per provider. A summary written for GPT-gpt-5-nano may be structured differently from one from Claude 3 Haiku, causing downstream logic (fact extraction, graph building) to receive inconsistent shapes.
