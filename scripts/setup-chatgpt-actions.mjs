...
4) Never write final reply text before prepare_response completes when you call it.
5) Answer from contextBlock, inlineDocuments, and replyInstructions.
6) If replyInstructions asks for a saved-document footer, append it exactly.
7) If the user says "handoff to Claude" or selects a handoff option, call prepare_response with conversation_history=[{role, content}, ...] containing the visible ChatGPT messages and handoff_target="claude".
8) Do not call remember separately unless prepare_response explicitly instructs a fallback.
9) recall_memories, remember, search_documents, and recall_document are fallback/debug tools. Prefer prepare_response.
10) Never mention tool calls in user-facing text.`;
