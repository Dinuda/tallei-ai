...
function BuilderToolPart({ part }: { part: DynamicToolUIPart }) {
...
export default function LoopBuilderPage() {
  ...
  async function handleRun() {
    ...
    else alert(`Run started: ${data.run?.id ?? "ok"}`);
  }
    ...
    <div className="mx-auto grid max-w-6xl gap-6 p-6 lg:grid-cols-[1fr_360px]">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-[#182506]">Loop builder</h1>
                ...
                <div className="max-w-full space-y-2">
                  {message.parts?.map((part, i) => {
                    ...
                    if (part.type.startsWith("tool-")) {
                      return <BuilderToolPart key={i} part={part as DynamicToolUIPart} />;
                    }
                    return null;
                  })}
                </div>
          ...
          <CardContent>
            ...
            {readyToCompile ? (
              <p className="mt-2 text-xs text-[#3d5c18]">
                Ask the builder to confirm connectors are connected, then Compile and Activate.
              </p>
            ) : null}
          </CardContent>
        </Card>
