...
export function formatRunFailureMessage(message: string | undefined | null): string {
  if (!message?.trim()) return "";
  return collapseValidationError(message);
}

export function resolveStepFailureMessage(
  step: SpecRunStep | undefined,
  runError?: string | null,
): string {
  const stepMessage = formatRunFailureMessage(step?.error_json?.message);
  if (stepMessage) return stepMessage;
  if (step?.status === "failed" || step?.status === "cancelled") {
    return formatRunFailureMessage(runError);
  }
  return "";
}
...
export function isRunMetaNarration(text: string): boolean {
  ...
  return /configured .*gate will now/.test(normalized)
    ...
    || /error from provider/i.test(normalized)
    || /maximum context length/i.test(normalized)
    || /source evidence agents must not/i.test(normalized)
    || /agent claimed an operator review/i.test(normalized)
    || /agent step .+ is failed/i.test(normalized);
}
