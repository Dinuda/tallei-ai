...
function phaseIcon(status: CleanupProposal["status"]) {
  if (status === "applied" || status === "approved") return <CheckCircle2 className="h-4 w-4 text-emerald-600" />;
  if (status === "failed" || status === "rejected") return <XCircle className="h-4 w-4 text-rose-600" />;
  if (status === "contested") return <GitMerge className="h-4 w-4 text-amber-600" />;
  if (status === "proposed") return <Sparkles className="h-4 w-4 text-slate-500" />;
