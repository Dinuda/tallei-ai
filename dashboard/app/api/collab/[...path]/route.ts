...
async function proxy(req: NextRequest, method: "GET" | "POST" | "DELETE", params: { path: string[] }) {
  ...
  const target = new URL(`${backend}/api/collab/${path}`);
  ...
  try {
    ...
    return Response.json(
      {
        error: isAbort
          ? `Timed out contacting backend /api/collab/${path}`
          : `Failed to reach backend /api/collab/${path}`,
      },
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  return proxy(req, "DELETE", await params);
}
