...
async function proxy(req: NextRequest, method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE"): Promise<Response> {
  ...
  try {
    const isStreamingChat = method === "POST" && path.endsWith("/run/chat");
    const body = method === "GET" ? undefined : isStreamingChat
      ? await req.text()
      : JSON.stringify(await req.json().catch(() => ({})));

    const res = isStreamingChat
      ? await fetch(target.toString(), { method, headers, body })
      : await fetchWithTimeout(target.toString(), { method, headers, body });
