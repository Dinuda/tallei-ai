...
export async function POST(
  ...
  try {
    const res = await fetchWithTimeout(
      `${backend}/api/integration-updates/${encodeURIComponent(assetKey)}/acknowledge`,
      {
        method: "POST",
        headers: {
    ...
    return Response.json(
      {
        error: isAbort
          ? "Timed out contacting backend /api/integration-updates/:assetKey/acknowledge"
          : "Failed to reach backend /api/integration-updates/:assetKey/acknowledge",
      },
