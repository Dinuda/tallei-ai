...
async function proxy(req: NextRequest, method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE"): Promise<Response> {
  ...
  const path = req.nextUrl.pathname.replace(/^\/api\/developer\/?/, "").replace(/^\/+/, "");
  const target = new URL(`${backend}/api/developer${path ? `/${path}` : ""}`);
  ...
  try {
    ...
    return Response.json(
      { error: isAbort ? "Timed out contacting backend developer API" : "Failed to reach backend developer API" },
