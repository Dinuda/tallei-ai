...
async function proxy(req: NextRequest, method: "GET" | "POST" | "DELETE"): Promise<Response> {
  ...
  const path = req.nextUrl.pathname.replace(/^\/api\/channels\/?/, "").replace(/^\/+/, "");
  const target = new URL(`${backend}/api/channels${path ? `/${path}` : ""}`);
  ...
  try {
    ...
    return Response.json(
      { error: isAbort ? "Timed out contacting backend channels API" : "Failed to reach backend channels API" },
