...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetchWithTimeout(`${backend}/api/claude-onboarding/sessions`, {
      method: "POST",
      headers: {
    ...
    return Response.json(
      {
        error: isAbort
          ? "Timed out contacting backend /api/claude-onboarding/sessions"
          : "Failed to reach backend /api/claude-onboarding/sessions",
      },
