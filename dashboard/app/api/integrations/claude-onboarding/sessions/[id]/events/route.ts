...
export async function GET(
  ...
  try {
    const res = await fetchWithTimeout(`${backend}/api/claude-onboarding/sessions/${id}/events`, {
      cache: "no-store",
      headers: {
    ...
    return Response.json(
      {
        error: isAbort
          ? "Timed out contacting backend /api/claude-onboarding/sessions/:id/events"
          : "Failed to reach backend /api/claude-onboarding/sessions/:id/events",
      },
