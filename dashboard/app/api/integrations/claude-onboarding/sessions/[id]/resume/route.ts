...
export async function POST(
  ...
  try {
    const res = await fetchWithTimeout(`${backend}/api/claude-onboarding/sessions/${id}/resume`, {
      method: "POST",
      headers: {
    ...
    return Response.json(
      {
        error: isAbort
          ? "Timed out contacting backend /api/claude-onboarding/sessions/:id/resume"
          : "Failed to reach backend /api/claude-onboarding/sessions/:id/resume",
      },
