...
const EmailEditor = dynamic(
  () => import("@react-email/editor").then((mod) => mod.EmailEditor),
  {
    ssr: false,
    loading: () => (
      <div className="flex min-h-[180px] flex-1 items-center justify-center text-sm text-[#6b7280]">
...
export function EmailArtifactEditorPanel({
  ...
  const saveButton = !hideSaveButton ? (
    <button
  ...
  return (
    <div
      className={cn(
        ...
        !inline && "rounded-xl border border-[#e5e7eb] shadow-sm",
      )}
    >
      {!inline ? (
      ...
      <div className={cn(
        "min-h-0 flex-1 overflow-auto bg-white [&_.ProseMirror]:min-h-[180px]",
        inline ? "px-1 py-0" : "p-2",
      )}
      >
        <EmailEditor
          content={content}
