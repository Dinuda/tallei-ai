...
export function BuilderConnectorChecklist({
  ...
  const confirmConnectedApps = useCallback(async () => {
  ...
  }, [onComplete, refresh, requirementId]);

  useEffect(() => {
    if (completed || !checklist?.complete || resolvingRef.current) return;
    void confirmConnectedApps();
  }, [checklist?.complete, completed, confirmConnectedApps]);
  ...
  if (completed) {
    return (
      <div className="my-3 flex items-center gap-3 border border-emerald-200 bg-emerald-50 px-4 py-3">
        ...
        <div>
          <div className="text-sm font-semibold text-emerald-950" style={{ fontFamily: "var(--font-title)" }}>Connection verified</div>
          <div className="text-xs text-emerald-700">
            The loop can use the selected connected apps.
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[13px] text-indigo-900/70">Connect the required app account so this loop can use it.</p>
            </div>
          </div>
