    ...
    return {
      ...
      afterFiles: [
        // ── Backend API & MCP (proxied transparently) ────────────────────────
        {
          ...
          // Keep local dashboard API handlers (e.g. /api/documents, /api/collab) in Next.js.
          // Proxy all other API routes to the backend.
          source:
            "/api/:path((?!(?:collab|documents|integrations|integration-updates|auth/(?:signin|signout|session|csrf|providers|callback|error|verify-request|webauthn-options))(?:/|$)).*)",
